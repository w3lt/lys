...
| Service tests pause an operation with explicit completion gates                  | A controlled schedule can prove that disposal waits and rejects later admission without relying on an arbitrary delay.                                                                                             |

The relevant sources are the [LLM service](../../apps/backend/src/di/services/llmService.ts), [service lifetime](../../apps/backend/src/di/singleton.ts), [chat task group](../../apps/backend/src/modules/chat/chat/index.ts), [desktop chat adapter](../../apps/desktop/src/lib/apis/http/chat.ts), [chat-view owner](../../apps/desktop/src/lib/store/chat-view/index.ts), [uptime hook](../../apps/desktop/src/lib/hooks/backendRuntime.ts), [model store](../../apps/desktop/src/lib/store/index.ts), and [service lifetime tests](../../apps/backend/test/di/services/llmService.test.ts).
