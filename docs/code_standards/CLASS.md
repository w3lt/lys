...
Renderer-managed state or lifecycle attached to a function or template component, or to a Hook satisfying `HOOK-001`, does not by itself create a repository-owned Class construct. An actual repository-owned instance that retains state, dependencies, resources, identity, or lifecycle across calls remains a Class construct and MUST follow this chapter. A hook-created controller, service, or closure with its own independent retained ownership remains subject to that boundary.
