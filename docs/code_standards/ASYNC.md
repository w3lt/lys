...
The alternative for work outliving its caller in `FUNC-017` does not waive `IFACE-018` when an interface operation can wait indefinitely. Platform limitations require a compliant boundary or the documented exception process; they do not create an implicit exemption. Lazy operation execution does not permit the fallible lazy initialization forbidden by `CLASS-012` or weaken readiness under `IFACE-019`.
...
An asynchronous owner MUST close admission before releasing dependencies that accepted work can still use. It MUST coordinate queued, active, deferred, and timer-started work according to its lifecycle contract, including the cancellation and awaiting of owned background work required by `CLASS-027`.
...
| -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The LLM runtime serializes accepted operations through a private completion tail | Keeping the queue usable after failure is separate from preserving each caller's original failure. Its current queue does not establish bounded admission or cancellable vendor waits. |
| Runtime disposal closes admission before awaiting accepted work                  | Joining accepted operations before dependency release demonstrates one shutdown boundary; it does not by itself establish every cancellation or deadline requirement.                  |
...
| A hook owns an uptime interval while the model store owns a transition timer     | Observer lifetime and application operation lifetime have different owners. The model transition is currently simulated, not evidence of a backend scheduler.                          |
| Runtime tests pause an operation with explicit completion gates                  | A controlled schedule can prove that disposal waits and rejects later admission without relying on an arbitrary delay.                                                                 |

The relevant sources are the [LLM runtime](../../apps/backend/src/di/services/lmStudioLlmRuntime.ts), [service lifetime](../../apps/backend/src/di/singleton.ts), [chat task group](../../apps/backend/src/modules/chat/chat/index.ts), [desktop chat adapter](../../apps/desktop/src/lib/apis/http/chat.ts), [chat-view owner](../../apps/desktop/src/lib/store/chat-view/index.ts), [uptime hook](../../apps/desktop/src/lib/hooks/backendRuntime.ts), [model store](../../apps/desktop/src/lib/store/index.ts), and [runtime lifetime tests](../../apps/backend/test/di/services/lmStudioLlmRuntime.test.ts).
